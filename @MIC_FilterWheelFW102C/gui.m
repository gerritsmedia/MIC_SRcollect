...
%gui Graphical User Interface to MIC_FilterWheelFW102C
%   GUI has functionality to change which filter is used

%   Gert-Jan Bakker Radboudumc 2018, modified from Marjolein Meddens, Lidke Lab 2017
...
obj.GuiFigure.Name = [obj.InstrumentName ' Id:' num2str(obj.Comport)];
...
for ii = 1 : numFilters
    buttonString = sprintf('%d: Fil %s ',ii,char(obj.Filters(ii)));
...
%% functions
    ...
    function properties2gui()
        ...
        handles.buttonGroup.SelectedObject = handles.buttonGroup.Children(tagNum == obj.CurrentFilterPos);
    end
